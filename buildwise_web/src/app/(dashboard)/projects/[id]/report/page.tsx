 ...
 <Worksheet ss:Name="BOQ Takeoff Report">
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#121216] pb-24 text-black dark:text-white antialiased">
      ...
      <div className="bg-white dark:bg-[#1E1E24] border-b border-black/[0.06] dark:border-white/[0.06] sticky top-0 z-40 print:hidden transition-all">
        <div className="max-w-[960px] mx-auto px-4 py-3.5 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2 rounded-xl border border-black/[0.08] dark:border-white/[0.08] hover:bg-black/[0.03] dark:hover:bg-white/[0.03]"
            >
      ...
      <div id="takeoff-report" className="max-w-[900px] mx-auto bg-white dark:bg-[#1E1E24] shadow-xl border border-black/[0.06] dark:border-white/[0.06] p-12 mt-8 space-y-12 print:shadow-none print:border-none print:p-0 print:mt-0 transition-all text-[12.5px] leading-relaxed">
        ...
        {/* Style block to handle print page breaks */}
        ...
        <div className="print-page-break h-[1050px] flex flex-col justify-between border-4 border-double border-violet-900/30 p-8 rounded-2xl relative">
          <div className="text-center pt-12 space-y-6">
            <div className="flex items-center justify-center gap-2 mb-4">
              ...
              <span className="text-xl font-black tracking-tight text-violet-600">BuildWise AI</span>
            </div>
            <p className="text-[11px] uppercase tracking-widest font-black text-violet-500">BIM Quantity Estimation & Takeoff Report</p>
            <h1 className="text-3xl font-black tracking-tight text-slate-800 dark:text-white uppercase pt-4">{project?.name || 'Omkar Koli'}</h1>
            <div className="w-20 h-1 bg-violet-600 mx-auto rounded-full" />
            <p className="text-[13px] text-black/50 dark:text-white/40 max-w-[400px] mx-auto italic">
              Standard quantity take-off document prepared in accordance with IS 1200 Indian standard measurement methodologies.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-8 border-t border-b border-black/[0.06] py-8 my-8 text-[12.5px]">
            <div className="space-y-3">
              <p><span className="font-bold text-black/40 uppercase text-[10px]">Client Name:</span><br /><span className="font-black text-[14px] text-violet-600">{metadata.clientName}</span></p>
              <p><span className="font-bold text-black/40 uppercase text-[10px]">Contractor:</span><br /><span className="font-semibold">{metadata.contractorName}</span></p>
              <p><span className="font-bold text-black/40 uppercase text-[10px]">Project Site:</span><br /><span>{metadata.address}, {metadata.city}, {metadata.state}, {metadata.country}</span></p>
            </div>
            <div className="space-y-3">
              <p><span className="font-bold text-black/40 uppercase text-[10px]">Architect:</span><br /><span className="font-semibold">{metadata.architectName}</span></p>
              <p><span className="font-bold text-black/40 uppercase text-[10px]">Engineer:</span><br /><span className="font-semibold">{metadata.engineerName}</span></p>
              <p><span className="font-bold text-black/40 uppercase text-[10px]">Prepared By:</span><br /><span className="italic">{metadata.preparedBy}</span></p>
            </div>
          </div>

          <div className="flex justify-between items-center text-[11px] text-black/40 pt-6">
            <span>Report Version: <strong>{metadata.version}</strong></span>
        ...
        <div className="print-page-break space-y-6">
          ...
          <div className="border border-black/[0.08] dark:border-white/[0.08] p-4 rounded-2xl bg-[#FAFAFC] dark:bg-black/20 flex flex-col items-center justify-center">
            {imageUrl ? (
              ...
              <img src={imageUrl} alt="Project Plan" className="max-h-[360px] object-contain rounded-lg" />
            ) : (
        ...
        {/* ── SECTION 4: AI DETECTION SUMMARY & SECTION 5: ROOM-WISE SUMMARY ── */}
        <div className="print-page-break space-y-6">
          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20">Section 4: AI Detection Metadata</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
            {[
              { label: 'Rooms Found', val: rooms.length },
              { label: 'Wall Segments', val: walls.length },
              { label: 'Doors Found', val: doors.length },
              { label: 'Windows Found', val: windows.length },
            ].map(i => (
              <div key={i.label} className="bg-slate-50 dark:bg-[#1E1E24] p-3 rounded-xl border border-black/[0.04] dark:border-white/[0.04]">
                <p className="text-[10px] text-black/40 dark:text-white/30 uppercase font-bold">{i.label}</p>
                <p className="text-[18px] font-black text-violet-500 mt-1">{i.val}</p>
              </div>
            ))}
          </div>

          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20 pt-4">Section 5: Room-wise Quantity Breakdown</h2>
        ...
        {/* ── SECTION 6: WALL-WISE STRUCTURAL TAKE-OFF ── */}
        <div className="print-page-break space-y-6">
          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20">Section 6: Wall-wise Structural Takeoff</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {wallWiseSummary.length === 0 && (
              <div className="col-span-2 text-center py-10 text-black/30 dark:text-white/20">No wall segments found or simulated.</div>
            )}
          </div>
        </div>

        {/* ── SECTION 7: MASTER BILL OF QUANTITIES (BOQ) ── */}
        <div className="print-page-break space-y-6">
          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20">Section 7: Master Bill of Quantities (BOQ)</h2>
        ...
        {/* ── SECTION 8: BRAND INFO & SECTION 9: LABOUR ESTIMATION ── */}
        <div className="print-page-break space-y-6">
          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20">Section 8: Material Brand Information</h2>
          ...
          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20 pt-4">Section 9: Labor Resource Estimation</h2>
          <div className="overflow-x-auto border border-black/[0.06] dark:border-white/[0.06] rounded-xl">
            <table className="w-full text-left border-collapse text-[11px]">
              <thead>
        ...
        {/* ── SECTION 10: EQUIPMENT & SECTION 11: COST BREAKDOWN ── */}
        <div className="print-page-break space-y-6">
          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20">Section 10: Equipment & Machinery Rental</h2>
          <div className="overflow-x-auto border border-black/[0.06] dark:border-white/[0.06] rounded-xl">
            <table className="w-full text-left border-collapse text-[11px]">
              <thead>
                <tr className="bg-slate-50 dark:bg-[#252530] font-bold text-[9px] uppercase tracking-wider text-black/50 dark:text-white/40 border-b border-black/[0.06] dark:border-white/[0.06]">
          ...
          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20 pt-4">Section 11: Total Takeoff Cost Analysis</h2>
        ...
        {/* ── SECTION 12: ROOM COST ANALYSIS & SECTION 13: GRAPHICAL ANALYSIS ── */}
        <div className="print-page-break space-y-6">
          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20">Section 12 & 13: Graphical cost analysis</h2>
        ...
        {/* ── SECTION 14: 3D VIEW SCREENSHOTS ── */}
        <div className="print-page-break space-y-6">
          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20">Section 14: 3D Building Mockup Views</h2>
          <div className="grid grid-cols-2 gap-4">
            <div className="border border-black/[0.05] dark:border-white/[0.05] p-2.5 rounded-xl bg-slate-50 dark:bg-black/10 flex flex-col items-center">
              <div className="w-full h-44 bg-slate-200 dark:bg-slate-800 rounded-lg flex items-center justify-center text-black/30 dark:text-white/20 font-bold">Complete Building Isometric View</div>
              <p className="text-[9.5px] text-black/40 dark:text-white/30 mt-1.5">Fig 1.2: Complete structural outer walls and layout view.</p>
            </div>
            <div className="border border-black/[0.05] dark:border-white/[0.05] p-2.5 rounded-xl bg-slate-50 dark:bg-black/10 flex flex-col items-center">
              <div className="w-full h-44 bg-slate-200 dark:bg-slate-800 rounded-lg flex items-center justify-center text-black/30 dark:text-white/20 font-bold">Orthographic Top / Plan View</div>
              <p className="text-[9.5px] text-black/40 dark:text-white/30 mt-1.5">Fig 1.3: Top slab removed view showing internal partitions.</p>
            </div>
          </div>
        </div>

        {/* ── SECTION 15: VALUE ENGINEERING & RECOMMENDATIONS ── */}
        <div className="print-page-break space-y-6">
          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20">Section 15: AI Suggestions &amp; Value Engineering</h2>
        ...
        {/* ── SECTION 16: ASSUMPTIONS & SECTION 17: DISCLAIMER ── */}
        <div className="space-y-6">
          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20">Section 16: Quantity Surveyor Assumptions</h2>
          ...
          <h2 className="text-[14px] font-black uppercase text-violet-500 tracking-wider pb-2 border-b-2 border-violet-500/20 pt-4">Section 17: Disclaimer &amp; Conditions</h2>
          <div className="p-4 bg-amber-500/[0.04] border border-amber-500/15 text-amber-700 dark:text-amber-500 rounded-2xl flex gap-3 text-[11.5px] leading-relaxed">
            <AlertTriangle className="w-5 h-5 shrink-0 mt-0.5" />
            <div className="space-y-1">
              <p className="font-bold uppercase tracking-wider text-[10px]">AI-Assisted Takeoff Verification Note</p>
              <p>This report is compiled utilizing advanced computer vision, OCR, and geometric analysis pipelines. Material quantities and civil cost break-ups are calculated from pixel coordinates, scale references, and user parameters.</p>
              <p className="font-bold">Final structural, reinforcement, and finishing quantities should always be verified by a licensed Civil Engineer or Chartered Quantity Surveyor before project execution or commercial bidding.</p>
            </div>
          </div>
        </div>

        {/* ── FINAL REPORT TOTALS SUMMARY ── */}
        <div className="border-t-4 border-double border-violet-600/40 pt-6 mt-12 flex flex-col items-end text-right">
